...
export default function CreatorDashboard() {
  ...
  const loadCreatorData = async () => {
    ...
    try {
      ...
      // Load creator stats from Supabase
      const [balance, streamsResult] = await Promise.all([
        // Get user's balance
        supabase
          .from('users')
          .select('alt_balance')
      ...
      if (balance.data) {
        setStats(prev => ({
          ...prev,
          currentBalance: balance.data.alt_balance || 0
        }));
      }

      if (streamsResult.data) {
        const creatorStreams = streamsResult.data;
        const totalEarnings = creatorStreams.reduce((sum, stream) => sum + (stream.earnings || 0), 0);
        const avgViewers = creatorStreams.length > 0 
          ? Math.round(creatorStreams.reduce((sum, stream) => sum + (stream.viewer_count || 0), 0) / creatorStreams.length)
          : 0;
        ...
        setStats(prev => ({
          ...
          upcomingStreams: creatorStreams.filter(s => s.status === 'scheduled').length
        }));

        setRecentStreams(creatorStreams);
      }
                    ...
                    {recentStreams.map((stream) => (
                      <div key={stream.id} className="flex items-center justify-between p-4 border dark:border-gray-700 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-900">
                        <div className="flex items-center">
                          ...
                          <div>
                            <h4 className="font-medium">{stream.title || 'Untitled Stream'}</h4>
